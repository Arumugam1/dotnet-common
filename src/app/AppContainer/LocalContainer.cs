...
using automation.components.data.v1.Local.Cassandra;
...
namespace automation.components.data.v1
{
    public static class LocalContainer
    {
        ...
        public static void InitWithInstallers()
        {
            Container.Init(Installer.WithInterfaces(typeof(ICoreComponents), typeof(IMockEdgeComponents)));
            if (!Initialized)
            {
                lock (lckInit)
                {
        ...
        {
            Container.Init(new LegacyInstaller());
            if (!Initialized)
                lock (lckInit)
                    if (!Initialized)
                    {
                ...
                lock (lckInit)
                    if (!Initialized)
                    {
                        try
                        {
                            ...
                            localDataExplorer._keyspace = tmpProvider.KeySpace;
                            localDataExplorer._columnFamily = tmpProvider.ColumnFamily;
                            localDataExplorer._keyPrefix = tmpProvider.KeyPrefix;

                            Container.RegisterAsForMock<IApplicationConfigProvider>(new ApplicationConfigProvider(localDataExplorer));

                            List<Entity> lstIOCs = JSon.Deserialize<List<Entity>>(Config.Manager.GetApplicationConfigValue("Entities", "AllApplications"));
                                ...
                                foreach (Entity ioc in lstIOCs)
                                {
                                    ...
                                    try
                                    {
                                        ...
                                        if ((typeof(Container).GetMethod("Resolve").MakeGenericMethod(interfaceType)
                                                ...
                                                && ((providerType.BaseType).FullName.Contains("automation.components.data.v1.Cassandra.DataExplorer")
                                                || (providerType.BaseType).FullName.Contains("automation.components.data.v1.MSSQL.DataExplorer")
                                                || (providerType.BaseType).FullName.Contains("automation.components.data.v1.ElasticSearch.Communicator")
                                                || (providerType.BaseType).FullName.Contains("automation.components.data.v1.Redis.DataExplorer"))
                                                || (providerType.BaseType).FullName.Contains("automation.components.data.v1.Redis.StackExchangeRedisProvider")
                                                || (providerType.BaseType).FullName.Contains("automation.components.data.v1.Cassandra.Communicator"))
                                            ...
                                            || ioc.providerFullName == "aric.variable.v1.manager.provider.cassandra.Provider")
                                        {
                                            ...
                                            if ((providerType.BaseType).FullName.Contains("automation.components.data.v1.Cassandra.DataExplorer"))
                                            {
                                                typeName =
                                                    (providerType.BaseType).FullName.Replace(
                                                        "automation.components.data.v1.Cassandra.DataExplorer`1[[",
                                                        "").Split(',')[0];
                                                ...
                                                o.GetType().GetProperty("_keyspace", BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Public)
                                                    .SetValue(o, defaultProvider.GetType().BaseType.GetProperty("KeySpace", BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Public).GetValue(defaultProvider));
                                                o.GetType().GetProperty("_columnFamily", BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Public)
                                                    .SetValue(o, defaultProvider.GetType().BaseType.GetProperty("ColumnFamily", BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Public).GetValue(defaultProvider));
                                                o.GetType().GetProperty("_keyPrefix", BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Public)
                                                    .SetValue(o, defaultProvider.GetType().BaseType.GetProperty("KeyPrefix", BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Public).GetValue(defaultProvider));
                                            }

                                            if (providerType.BaseType.FullName.Contains("automation.components.data.v1.MSSQL.DataExplorer"))
                                            {
                                                typeName =
                                                    (providerType.BaseType).FullName.Replace(
                                                        "automation.components.data.v1.MSSQL.DataExplorer`1[[", "")
                                                        .Split(',')[0];

                                                var d1 = typeof(Local.MSSQL.DataExplorer<>);
                                                Type[] typeArgs = { Assembly.LoadFrom(_assemblyPath).GetType(typeName) };
                                                var makeme = d1.MakeGenericType(typeArgs);
                                                o = Activator.CreateInstance(makeme);
                                            }

                                            if ((providerType.BaseType).FullName.Contains("automation.components.data.v1.Redis.DataExplorer"))
                                            {
                                                typeName =
                                                    (providerType.BaseType).FullName.Replace(
                                                        "automation.components.data.v1.Redis.DataExplorer`1[[", "")
                                                        .Split(',')[0];
                                            ...
                                            if ((providerType.BaseType).FullName.Contains("automation.components.data.v1.Redis.StackExchangeRedisProvider"))
                                            {
                                                typeName =
                                                    (providerType.BaseType).FullName.Replace(
                                                        "automation.components.data.v1.Redis.StackExchangeRedisProvider`1[[", "")
                                                        .Split(',')[0];
                                                var d1 = typeof(Local.Redis.DataExplorer<>);
                                                if (typeName.Contains("System.Collections.Hashtable"))
                                                {
                                            ...
                                            if ((providerType.BaseType).FullName.Contains("automation.components.data.v1.ElasticSearch.Communicator"))
                                            {
                                                typeName =
                                                    (providerType.BaseType).FullName.Replace(
                                                        "automation.components.data.v1.ElasticSearch.Communicator`1[[", "")
                                                        .Split(',')[0];
