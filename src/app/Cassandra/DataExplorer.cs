...
using automation.core.components.data.v1.Buffer;
...
using Cassandra;
using Trace = automation.core.components.operations.v1.Trace;
...
namespace automation.core.components.data.v1.Cassandra
{
    ...
    public abstract class DataExplorer<T> : IDataExplorer<T>, IDisposable
    {
        ...
        protected static Dictionary<string, Entity<T>> bufferDB
                 = new Dictionary<string, Entity<T>>();
        ...
        public virtual List<T> GetIndexes(string columnFamily, List<string> returnColumn, Dictionary<string, string> whereColumnValues, int limit)
        {
            ...
            {
                string cqlQuery = string.Format("select {2} from {0}.{1} where {3} {4};", this._keyspace, columnFamily,
        ...
        public virtual bool DeleteIndex(string columnFamily, Dictionary<string, object> whereColumnValues)
        {
            ...
            string cqlQuery = string.Format("delete from {0}.{1} where {2} = ?;", this._keyspace, columnFamily, string.Join(" = ? AND ", whereColumnValues.Keys));
            ...
            Dictionary<string,bool> autoTimeUUIDValues)
        {
            try
            {
                ...
                string cqlQuery = string.Format("update {0}.{1} set {2} = ? {4}{5} where {3} = ?", this._keyspace,
            ...
            catch (Exception ex)
            {
                Trace.Exception(TraceTypes.AccessLayer,
                    "automation.core.components.data.v1.Cassandra.DataExplorer<T>",
                    "Update",
                ...
                foreach (var row in indexes)
                {
                    ...
                    switch (row.Type)
                    {
                        case CounterType.Increment:
                            cqlQuery = string.Format("update {0}.{1} set {2} = {2} + {3} where {4} = ?", this._keyspace,
        ...
        public virtual Dictionary<string, object> GetDataAsObject(string columnFamily, List<string> keys, string valueColumn)
        {
            ...
            var results =
                this.JoinedAsyncQuery(
                    string.Format("select key, {2} from {0}.{1} where key=? limit {3};", this._keyspace, columnFamily,
        ...
        public virtual Dictionary<string, object> GetDataAsObject(string columnFamily, List<string> keys, string valueColumn, string keyColumn)
        {
            ...
            var results =
                this.JoinedAsyncQuery(
                    string.Format("select {2}, {3} from {0}.{1} where {2}=? limit {4};", this._keyspace, columnFamily,
        ...
        {
            return this.Add(new List<Data> { new Data { Key = key, Value = value } }, 0);
        }

        public virtual bool Add(string key, T value, uint timeToLive)
        {
            return this.Add(new List<Data> { new Data { Key = key, Value = value } }, timeToLive);
        }

        public virtual bool Add(Data data)
        {
            return this.Add(new List<Data> { data }, 0);
        }

        public virtual bool Add(Data data, uint timeToLive)
        {
            return this.Add(new List<Data> { data }, timeToLive);
        }
        ...
        public virtual bool Add(List<Data> data, uint timeToLive)
        {
            try
            {
                ...
                var formattedString = string.Format(statementString, this._keyspace, this._columnFamily);
                ...
                WaitUnlessFault(
                    data.Select(
                        curData =>
                            timeToLive == 0
                                ? statement.Bind(BuildBindVars(this._columnFamily, curData.Key,
        ...
        {
            return this.Get(new List<string> { key }).FirstOrDefault();
        }
        ...
        {
            return this.Get(new List<string> { key }, executeInSingleNode).FirstOrDefault();
        }

        public virtual List<T> Get(int limit)
        {
            var results = this.ExecuteQuery(string.Format("select value from {0}.{1} {2};", this._keyspace, this._columnFamily, this.GetLimitQuery(limit)));

            return this.TransformRowsAndDeserialize(results, "value");
        }
        ...
        public virtual List<string> GetKeys()
        {
            var st = DateTime.UtcNow;
            var results =
                this.ExecuteQuery(string.Format("select key from {0}.{1} limit {2};", this._keyspace,
                    this._columnFamily, int.MaxValue));
        ...
        {
            return this.GetData(keys, false);
        }

        public virtual List<T> Get(List<string> keys, bool executeInSingleNode)
        {
            return this.GetData(keys, executeInSingleNode);
        }
        ...
        /// <param name="executeInSingleNode"></param>
        /// <returns></returns>
        private List<T> GetData(List<string> keys, bool executeInSingleNode)
        {
            if (keys == null || !keys.Any())
                return new List<T>();
            ...
            var results = this.JoinedAsyncQuery(
                string.Format(
                    "select value from {0}.{1} where key=? limit {2};", this._keyspace, this._columnFamily, keys.Count), keys, executeInSingleNode);

            List<T> colVals = this.TransformRowsAndDeserialize(results, "value");
        ...
        private string GetDataAsString(string key, bool executeInSingleNode)
        {
            ...
            if (this.UsePreparedStatementGetCalls)
            {
                results =
                    this.JoinedAsyncQuery(
                        string.Format("select value from {0}.{1} where key=? ;", this._keyspace, this._columnFamily
            ...
            else
            {
                results =
                    this.ExecuteQuery(
                        string.Format("select value from {0}.{1} where key  =  '{2}';", this._keyspace,
                            this._columnFamily, key), executeInSingleNode);
            }
        ...
        {
            return this.GetDataAsString(this._columnFamily, keys, false);
        }

        public virtual T Get(string columnFamily, string key)
        {
            return this.Get(columnFamily, new List<string> { key }).FirstOrDefault();
        }
        ...
        {
            return this.GetDataAsString(this._columnFamily, keys, executeInSingleNode);
        }
        ...
        public virtual List<T> Get(string columnFamily, List<string> keys)
        {
            ...
            var results =
                this.JoinedAsyncQuery(
                    string.Format("select value from {0}.{1} where key=? ;", this._keyspace, columnFamily), keys);
        ...
        private Dictionary<string, string> GetDataAsString(string columnFamily, List<string> keys, bool executeInSingleNode)
        {
            ...
            var results =
                this.JoinedAsyncQuery(
                    string.Format("select key, value from {0}.{1} where key=? limit {2};", this._keyspace, columnFamily,
        ...
        private Dictionary<string, string> GetDataAsString(string columnFamily, int limit, bool executeInSingleNode)
        {
            ...
            var results =
                this.ExecuteQuery(
                    string.Format("select key, value from {0}.{1} {2};", this._keyspace, columnFamily,
        ...
        public virtual List<string> Filter(string columnName, DateTime from, DateTime to, int limit)
        {
            ...
            var results =
                this.ExecuteQuery(string.Format("select key from {0}.{1}{2} {3} ALLOW FILTERING;", this._keyspace,
                    this._columnFamily, this.GetFromAndToQuery(columnName, " where ", from, to),
                    this.GetLimitQuery(limit)));
        ...
        public virtual List<T> GetAllRows(string columnFamily, int limit)
        {
            ...
            columnFamily = this.ValidateName("columnFamily", columnFamily);
            var results = this.ExecuteQuery(string.Format("select * from {0}.{1} {2};", this._keyspace, columnFamily, this.GetLimitQuery(limit)));

            var objectList = this.TransformRowsAndDeserialize(results, "value");
        ...
        public virtual bool Delete(string key)
        {
            ...
            this.ExecuteNonQuery(string.Format("delete from {0}.{1} where key='{2}';", this._keyspace, this._columnFamily, key));
        ...
        public virtual bool AddIndex(string columnFamily, Dictionary<string, string> keyValues, uint timeToLive)
        {
            try
            {
                ...
                var statement =
                    this.CachePrepare(string.Format("insert into {0}.{1} (key, value) values (?, ?)", this._keyspace,
                        columnFamily));
                var statementTtl =
                    this.CachePrepare(string.Format("insert into {0}.{1} (key, value) values (?, ?) USING TTL ?", this._keyspace,
                        columnFamily));
        ...
        public virtual bool AddIndexes(string columnFamily, List<Index> indexes, uint timeToLive)
        {
            try
            {
                ...
                var statement =
                   this.CachePrepare(string.Format("insert into {0}.{1} (key, column1, value) values (?, ?, ?)", this._keyspace,
                       columnFamily));
                ...
                foreach (var keyItem in indexes)
                {
                    ...
                    foreach (var item in keyItem.KeyValues)
                    {
                        ...
                        {
                            var statementTtl = this.CachePrepare(string.Format("insert into {0}.{1} (key, column1, value) values (?, ?, ?) USING TTL {2}", this._keyspace,
        ...
        public virtual bool AddIndexesWithTimeUUID(string columnFamily, List<Index> indexes, uint timeToLive)
        {
            try
            {
                ...
                var statement =
                   this.CachePrepare(string.Format("insert into {0}.{1} (key, column1, value, modification_timestamp) values (?, ?, ?, now())", this._keyspace,
                       columnFamily));
                ...
                foreach (var keyItem in indexes)
                {
                    ...
                    foreach (var item in keyItem.KeyValues)
                    {
                        ...
                        {
                            var statementTtl = this.CachePrepare(string.Format("insert into {0}.{1} (key, column1, value, modification_timestamp) values (?, ?, ?, now()) USING TTL {2}", this._keyspace,
                                                                                         columnFamily, (int)(timeToLive + keyItem.TimeToLive)));
        ...
        public virtual bool AddIndexes(string columnFamily, List<TimeUUIDIndex> indexes, uint timeToLive)
        {
            try
            {
                ...
                foreach (var keyItem in indexes)
                {
                    ...
                    foreach (var item in keyItem.KeyValues)
                    {
                        ...
                        {
                            var statementTtl =
                                this.CachePrepare(string.Format("insert into {0}.{1} (key, column1, value) values (?, ?, ?) USING TTL {2}", this._keyspace,
                                                          columnFamily, (int)(timeToLive + keyItem.TimeToLive)));
                            bind = statementTtl.Bind(BuildBindVars(columnFamily, key, curColumnName, curColumnValue));
                        }
                        var resultSetFuture = session.ExecuteAsync(bind);
            ...
            {
                Trace.Exception(TraceTypes.AccessLayer,
                    "automation.core.components.data.v1.Cassandra.Communicator<T>",
                    "AddIndexes",
                    new object[] { columnFamily, indexes, timeToLive },
                    ex);
        ...
        public virtual Dictionary<string, string> GetIndex(string columnFamily, string key, int limit)
        {
            ...
            if (this.UsePreparedStatementGetCalls)
            {
                results =
                    this.JoinedAsyncQuery(
                        string.Format("select column1,value from {0}.{1} where key=? {2};", this._keyspace, columnFamily,
                            this.GetLimitQuery(limit)
                            ), new List<string>() {key});
            }
            else
            {
                results =
                    this.ExecuteQuery(string.Format("select column1,value from {0}.{1} where key='{2}' {3};",
                        this._keyspace, columnFamily, key, this.GetLimitQuery(limit)));
            }
        ...
        public virtual Dictionary<string, string> GetUniqueIndex(string columnFamily, string key, int limit)
        {
            ...
            if (this.UsePreparedStatementGetCalls)
            {
                results =
                    this.JoinedAsyncQuery(
                        string.Format("select column1,value from {0}.{1} where key=? {2};", this._keyspace,
                            this._columnFamily,
        ...
        public virtual Dictionary<string, string> GetIndexesColumns(string columnFamily, int limit)
        {
            ...
            var results =
                this.ExecuteQuery(string.Format("select key, column1 from {0}.{1} {2};", this._keyspace, columnFamily,
                    this.GetLimitQuery(limit)));
        ...
        public virtual List<Index> GetIndexes(string columnFamily, int limit)
        {
            ...
            columnFamily = this.ValidateName("columnFamily", columnFamily);
            var results =
                this.ExecuteQuery(string.Format("select key, column1,value from {0}.{1} {2};", this._keyspace,
        ...
        public virtual List<Index> GetIndexes(string columnFamily, List<string> keys)
        {
            ...
            var results = this.JoinedAsyncQuery(
                String.Format(
                    "select key, column1,value from {0}.{1} where key=?", this._keyspace, columnFamily),
                keys);
        ...
        public virtual List<string> GetIndexColumnNames(string columnFamily, string key, int limit)
        {
            ...
            if (this.UsePreparedStatement)
            {
                results =
                    this.JoinedAsyncQuery(
                        string.Format("select column1 from {0}.{1} where key=? {2};", this._keyspace, columnFamily,
                             this.GetLimitQuery(limit)), new List<string>() {key});
            }
            else
            {
                results =
                    this.ExecuteQuery(string.Format("select column1 from {0}.{1} where key='{2}' {3};", this._keyspace,
        ...
        public virtual List<string> GetIndexColumnValues(string columnFamily, string key, int limit)
        {
            ...
            {
                 results =
                this.ExecuteQuery(string.Format("select value from {0}.{1} where key='{2}' {3};", this._keyspace,
                    columnFamily, key, this.GetLimitQuery(limit)));
            }
                ...
                results =
                    this.ExecuteQuery(
                        string.Format("select column1, value from {0}.{1} where key='{2}' and column1='{4}' {3};",
                            this._keyspace, columnFamily, key, this.GetLimitQuery(limit), indexKey));
            }
        ...
        public virtual Dictionary<string, string> GetTimeIndex(string columnFamily, string columnName, string key, string from, string to, int limit)
        {
            ...
            if (this.UsePreparedStatementGetCalls)
            {
                results =
                    this.JoinedAsyncQuery(
                        string.Format("select {2},value from {0}.{1} where key=? {3} {4};", this._keyspace,
            ...
            {
                results = this.ExecuteQuery(string.Format("select {2},value from {0}.{1} where key='{3}'{4} {5};", this._keyspace,
                ...
                results =
                    this.JoinedAsyncQuery(
                        string.Format("select {2}, value from {0}.{1} where key=? {3} order by {2} {5} {4};",
                            this._keyspace, columnFamily, columnName,
                ...
                results =
                    this.ExecuteQuery(
                        string.Format("select {2}, value from {0}.{1} where key='{3}'{4} order by {2} {6} {5};",
                            this._keyspace, columnFamily, columnName, key,
                            this.GetFromAndToQuery(columnName, " AND ", from, to), this.GetLimitQuery(limit),
                            ascending ? "asc" : "desc"));
            }
            var colVals = this.TransformRows<Guid>(results, columnName, "value");
        ...
        public virtual List<string> GetTimeIndexColumnNames(string columnFamily, string columnName, string key, string from, string to, int limit)
        {
            ...
            if (this.UsePreparedStatement)
            {
                results =
                    this.JoinedAsyncQuery(
                        string.Format("select {2} from {0}.{1} where key=? {3} {4};", this._keyspace,
                            columnFamily, columnName, this.GetFromAndToQuery(columnName, " AND ", from, to),
                            this.GetLimitQuery(limit)), new List<string>() {key});
            }
            else
            {
                results =
                    this.ExecuteQuery(string.Format("select {2} from {0}.{1} where key='{3}'{4} {5};", this._keyspace,
        ...
        public virtual List<string> GetTimeIndexColumnNames(string columnFamily, string columnName, string key, string from, string to, int limit, bool ascending)
        {
            ...
            else
            {
                ...
                this.ExecuteQuery(string.Format("select {2} from {0}.{1} where key='{3}'{4} order by {2} {6} {5};",
                    this._keyspace, columnFamily, columnName, key, this.GetFromAndToQuery(columnName, " AND ", from, to),
        ...
        public virtual List<string> GetTimeIndexColumnValues(string columnFamily, string columnName, string key, string from, string to, int limit)
        {
            ...
            if (this.UsePreparedStatement)
            {
                results =
                    this.JoinedAsyncQuery(
                        string.Format("select value from {0}.{1} where key=? {2} {3};", this._keyspace,
            ...
            {
                results =
                    this.ExecuteQuery(string.Format("select value from {0}.{1} where key='{2}'{3} {4};", this._keyspace,
        ...
        public virtual List<string> GetTimeIndexsColumnValues(string columnFamily, string columnName, string keys, string from, string to, int limit, bool ascending)
        {
            ...
            var results =
                this.ExecuteQuery(string.Format("select value from {0}.{1} where key in {3}{4} order by {2} {6} {5};",
                    this._keyspace, columnFamily, columnName, keys,
                    this.GetFromAndToQuery(columnName, " AND ", from, to), this.GetLimitQuery(limit),
                    ascending ? "asc" : "desc"));
        ...
        public virtual List<string> GetTimeIndexsColumnValues(string columnFamily, string columnName, List<string> keys, string from, string to, int limit, bool ascending)
        {
            ...
            var results =
                this.JoinedAsyncQuery(
                    string.Format("select value from {0}.{1} where key=? {3} order by {2} {5} {4};", this._keyspace,
                        columnFamily, columnName, this.GetFromAndToQuery(columnName, " AND ", from, to),
                        this.GetLimitQuery(limit),
        ...
        public virtual bool DeleteIndex(string columnFamily, string key)
        {
            ...
            this.ExecuteNonQuery(string.Format("delete from {0}.{1} where key='{2}';", this._keyspace, columnFamily, key));
        ...
        public virtual bool DeleteIndex(string columnFamily, string columnName, string key, string indexKey)
        {
            ...
            this.ExecuteNonQuery(string.Format("delete from {0}.{1} where key='{2}' and {3}='{4}';", this._keyspace, columnFamily, key, columnName, indexKey));
        ...
        public virtual int Count(string columnFamily, string key, int limit)
        {
            ...
            if (this.UsePreparedStatement)
            {
                results =
                    this.JoinedAsyncQuery(
                        string.Format("select count(*) from {0}.{1} where key=? {2};", this._keyspace,
                            columnFamily, this.GetLimitQuery(limit)), new List<string>() {key});
            }
            else
            {
                results =
                    this.ExecuteQuery(string.Format("select count(*) from {0}.{1} where key='{2}' {3};", this._keyspace,
                        columnFamily, key, this.GetLimitQuery(limit)));
            }
        ...
        public virtual List<T> TransformRowsAndDeserialize(IEnumerable<Row> rows, string desiredColumn)
        {
        ...
        {
            return DataStaxConnection.Session;
        }
        ...
        public static void WaitUnlessFault(Task[] tasks, CancellationToken token)
        {
            ...
            catch (OperationCanceledException ex)
            {
                ...
                if (faultedTask != null)
                {
                    ...
                    Trace.Exception(TraceTypes.AccessLayer,
                    "automation.core.components.data.v1.Cassandra.DataExplorer",
                    "FaultedTask",
            ...
            bool executeWithCLLocalQuorum)
        {
            ...
            var rows = this.JoinedAsyncQuery(columnFamily, new List<string>() {requestColumn}, whereClause, limit,
                false, executeWithCLOne, executeWithCLLocalQuorum);
        ...
        public bool ExecuteNonQuery(string columnFamily, List<RowIndex> indexes, uint timeToLive)
        {
            try
            {
                ...
                foreach (var row in indexes)
                {
                    ...
                    {
                        cqlQuery = string.Format("insert into {0}.{1} ({2}{4}{6}) values ({3}{5}{7})", this._keyspace,
                    ...
                    else
                    {
                        cqlQuery = string.Format("insert into {0}.{1} ({2}{4}{7}) values ({3}{5}{8}) USING TTL {6}",
                            this._keyspace, columnFamily, string.Join(", ", row.KeyValues.Keys),
                            string.Join(", ", items),
                            row.EnableAutoTimeUUID ? ", modification_timestamp" : string.Empty,
                            row.EnableAutoTimeUUID ? ", now()" : string.Empty,
        ...
        private void ReportSlowQueries(string methodName, string query, DateTime requestTime, DateTime responseTime)
        {
            ...
            {
                Trace.Exception(TraceTypes.AccessLayer,"automation.core.components.data.v1.Cassandra.DataExplorer<T>","ReportSlowQueries", new object[] { query }, ex);
            }
        }
        ...
        private void RecordConnectionFailed()
        {
            ...
            {
                Trace.Exception(TraceTypes.AccessLayer, "automation.core.components.data.v1.Cassandra.DataExplorer<T>", "RecordConnectionFailed", new object[] { }, ex);
            }
        }
        ...
        private void RecordConnectionFailed(string errorMessage)
        {
            ...
            {
                Trace.Exception(TraceTypes.AccessLayer, "automation.core.components.data.v1.Cassandra.DataExplorer<T>", "RecordConnectionFailed", new object[] { errorMessage }, ex);
            }
        }

        private List<Row> JoinedAsyncQuery(string query, List<string> partitionKeys, bool executeWithCLOne = false)
        {
            ...
            bool executeWithCLLocalQuorum = false)
        {
            ...
            foreach (var columns in whereClause.Values)
            {
                ...
                if (selectJson)
                {
                    cqlQuery = string.Format("select JSON {2} from {0}.{1} where {3} {4};", this._keyspace, columnFamily,
                        returnColumn == null || !returnColumn.Any() ? "*" : string.Join(", ", returnColumn),
                ...
                {
                    cqlQuery = string.Format("select {2} from {0}.{1} where {3} {4};", this._keyspace, columnFamily,
                        returnColumn == null || !returnColumn.Any() ? "*" : string.Join(", ", returnColumn),
                        string.Join(" and ", columns.Select(x => string.Format("{0} = ?", x.Key, x.Value))),
                        this.GetLimitQuery(limit));
                }
        ...
        {
            return this.Get(columnFamily, keys);
        }
