...
namespace automation.components.data.v1.Cassandra
{
    public interface IDataExplorer<T>
    {
        ...
        List<T> GetAllRows(string columnFamily, List<string> requestColumn, int limit);
        ...
        /// <summary>
        /// Warning: "Select JSON ..." option to get JSON object from columns in select query is converting empty strings to null values.
        /// To void this conversion, retrieve original row data using ExecuteQueryToGetOriginalDataRows. For solr querries do not forget to set "executeWithCLLocalOne" to "true"
        /// </summary>
        ...
        List<T> ExecuteQueryToGetOriginalDataRows(string columnFamily, List<string> requestColumn, Dictionary<int, Dictionary<string, object>> whereClause, int limit, bool executeWithCLOne = false, bool executeWithCLLocalQuorum = false, bool executeWithCLLocalOne = false);
