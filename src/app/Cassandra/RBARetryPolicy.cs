...
namespace automation.core.components.data.v1.Cassandra
{
    ...
    /// Blurb
    /// </summary>
    class RBARetryPolicy : IRetryPolicy
    {
        ...
        public RBARetryPolicy(int maximumReadAttempts, int maximumWriteAttempts, int maximumUnavailableAttempts, int backoff,
            bool graphEnabled = true, bool traceEnabled = true)
        {
            ...
            if(graphEnabled)
                this.Graph = Container.Resolve<IGraphProvider>();
        ...
        private void TraceInit()
        {
            if (this.traceEnabled)
                Trace.WriteLine(
                      String.Format("RBARetryPolicy initialized maximums read/write/conn: {0}/{1}/{2}, backoff: {3}", this.maxRead, this.maxWrite, this.maxUnavailable, this.BackoffTime
                          ));
        }
        ...
        private void TraceRead(ConsistencyLevel cl, int required, int recieved, bool data, int retries)
        {
            ...
            this.RecordTraceMetric(Constants.RBARetryPolicy, (int)cl, required, recieved, retries);
        }
        ...
        private void TraceWrite(ConsistencyLevel cl, string writeType, int requiredAcks, int receivedAcks, int nbRetry)
        {
            ...
            this.Dispatch(message);
            this.RecordTraceMetric(Constants.RBARetryPolicy_Ack, (int)cl, requiredAcks, receivedAcks, nbRetry);
        }
        ...
        private void TraceUnavailable(ConsistencyLevel cl, int requiredReplica, int aliveReplica, int nbRetry)
        {
            ...
            this.Dispatch(message);
            this.RecordTraceMetric(Constants.RBARetryPolicy_Repl, (int)cl, requiredReplica, aliveReplica, nbRetry);
        }

        private void RecordTraceMetric(string metric, int cl, int req, int rec, int retry)
        {
            if (this.Graph != null)
            {
                this.Graph.RecordAt(new Point(String.Concat(metric, ".", Constants.RBARetryPolicy_ConsistencyLevel), (int)cl));
                this.Graph.RecordAt(new Point(String.Concat(metric, ".", Constants.RBARetryPolicy_RequiredCount), req));
                this.Graph.RecordAt(new Point(String.Concat(metric, ".", Constants.RBARetryPolicy_ReceivedCount), rec));
                this.Graph.RecordAt(new Point(String.Concat(metric, ".", Constants.RBARetryPolicy_RetriesCount), retry));
            }
        }
        ...
        private void TraceChangeConsistencyLevel(ConsistencyLevel cl)
        {
            ...
            if (this.Graph != null)
            {
                this.Graph.RecordAt(new Point(String.Concat(Constants.RBARetryPolicy, ".", Constants.RBARetryPolicy_ConsistencyLevel), (int)cl));
            }
        }
