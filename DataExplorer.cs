...
using automation.core.components.data.v1.CustomTypes;
...
namespace automation.core.components.data.v1.Redis
{
    public abstract class DataExplorer<T> : IDataExplorer<T>, IDisposable
    {
        ...
        public static ConnectionMultiplexer Connection
        {
            get
            {
                return masterRedisConnection.Value;
            }
        }
        ...
        {
            return this.Get(new List<string> { key }, CommandFlags.None).FirstOrDefault();
        }
        ...
        private void ReportSlowQueries(string methodName, string query, DateTime requestTime, DateTime responseTime)
        {
            ...
            try
            {
                ...
                Trace.Warning(
                    TraceTypes.AccessLayer,
                    "automation.core.components.data.v1.Redis.Communicator<T>",
                    methodName,
                    traceObj);
            }
            catch (Exception ex)
            {
                Trace.Exception(TraceTypes.AccessLayer,
                    "automation.core.components.data.v1.Redis.DataExplorer",
                    "ReportSlowQueries",
                    new object[] { query },
                    ex);
            }
